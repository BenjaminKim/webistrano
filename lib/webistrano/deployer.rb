module Webistrano
  class Deployer
    ...
    def initialize(deployment)
      ...
      if @deployment.task and not @deployment.new_record?
        # a read deployment
    ...
    def execute!
      ...
      status = catch(:abort_called_by_capistrano) do
        ...
        if config.variables[:skip_krb] != false && ENV["KRB5CCNAME"]
          FileUtils.remove_file(ENV["KRB5CCNAME"])
        end
      end
